...
import { trackOrder, getOrderById } from '@/lib/api/orders'
...
function TrackOrderContent() {
  ...
  // Pre-fill phone if user is authenticated
  useEffect(() => {
    if (user?.phone && !phone) {
      setPhone(user.phone)
    }
  ...
  // Handle direct links: ?orderId=... or ?orderNumber=...&phone=...
  useEffect(() => {
    const directOrderId = searchParams.get('orderId') || searchParams.get('id')
    const paramOrderNumber = searchParams.get('orderNumber') || searchParams.get('order')
    const paramPhone = searchParams.get('phone')

    if (directOrderId) {
      setLoading(true)
      setSearched(true)
      getOrderById(directOrderId)
        .then((data) => {
          if (data) {
            setOrder(data)
            setOrderNumber(data.orderNumber)
          } else {
            setError('Could not locate the requested order.')
          }
        })
        .catch((err) => {
    ...
    if (paramOrderNumber) {
      ...
      if (paramPhone) {
        ...
        setLoading(true)
        setSearched(true)
        trackOrder({ orderNumber: paramOrderNumber, phone: paramPhone })
          .then((data) => {
            if (data) setOrder(data)
            else setError(`No order found matching "${paramOrderNumber}" and phone "${paramPhone}".`)
          })
          .catch((err) => {
  ...
  const handleTrack = async (e) => {
    ...
    if (!cleanPhone) {
      toast.error('Please enter the Phone Number used at checkout.')
      return
    }
    ...
    try {
      const data = await trackOrder({ orderNumber: cleanNumber, phone: cleanPhone })
      if (!data) {
        setError(`No order found matching Order #${cleanNumber} and phone "${cleanPhone}". Please check the details from your order confirmation.`)
      } else {
        setOrder(data)
      ...
      setError(err?.message || 'Could not retrieve order details. Please verify your order number and phone number.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="py-12 md:py-16 relative">
      <div className="container max-w-4xl px-4 relative z-10">
        <div className="text-center mb-8">
          <h1 className="font-display text-3xl md:text-4xl text-stone-900 font-bold">Track Your Order</h1>
          <p className="text-stone-500 text-sm mt-1.5 font-inter max-w-sm mx-auto">
            Enter your order details below to view delivery status.
          </p>
        </div>
            ...
            <div className="bg-white border border-stone-200 rounded-2xl p-6 md:p-8 shadow-sm">
              <form onSubmit={handleTrack} className="space-y-4">
                <div className="space-y-1.5">
                  <Label htmlFor="orderNumber" className="text-stone-800 font-semibold font-inter text-xs uppercase tracking-wide">
                    Order Number
                  </Label>
                  <Input
                    id="orderNumber"
                    ...
                    placeholder="ORD-1790... or Order ID"
                    required
                ...
                <div className="space-y-1.5">
                  <Label htmlFor="phone" className="text-stone-800 font-semibold font-inter text-xs uppercase tracking-wide">
                    Phone Number
                  </Label>
                  <div className="relative">
                    ...
                    <Input
                      ...
                      required
                      className="pl-9 bg-stone-50/80 border-stone-200 focus-visible:ring-saffron text-stone-900 placeholder:text-stone-400 h-11 text-sm font-inter"
                    />
                  </div>
                </div>
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-stone-100 pb-5">
                <div>
                  ...
                  <div className="flex items-center gap-2 flex-wrap mt-0.5">
                    <h2 className="font-display text-2xl font-bold text-stone-900">#{order.orderNumber || order.id?.slice(0, 8)}</h2>
                    {order.id && (
              ...
              {!isTerminalCancelledOrRefunded ? (
                <div className="py-6 border-b border-stone-100">
                  ...
                  {order.statusHistory?.[0]?.note && (
                    <div className="mt-6 p-3.5 bg-stone-50 rounded-xl border border-stone-200/80 text-xs text-stone-700 flex items-start gap-2.5 max-w-xl mx-auto font-inter">
                      <Truck className="w-4 h-4 text-[#FF6B00] shrink-0 mt-0.5" />
                      <div>
                        <p className="font-semibold text-stone-900">Dispatch / Transit Note</p>
                        <p className="text-stone-600 mt-0.5">{order.statusHistory[0].note}</p>
                      </div>
                    </div>
                  )}
                </div>

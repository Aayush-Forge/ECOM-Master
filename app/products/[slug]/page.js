...
function ProductDetail() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/products/${slug}`).then(r => r.json()).then(data => {
      if (!data.error) {
        setProduct(data)
        const def = {}
        if (data.default_attributes?.length) {
          for (const a of data.default_attributes) def[a.name] = a.option
        }
        setSelectedAttrs(def)
        setLoading(false)
        fetchReviews(data.id)

        // Update history cache in localStorage
        try {
          const viewedHistory = JSON.parse(localStorage.getItem('sd_viewed_history') || '[]')
          if (!viewedHistory.includes(data.id)) {
            viewedHistory.push(data.id)
  ...
  const matchedVariation = useMemo(() => {
    ...
    return product.variationsData.find(v => {
      // every required attr must match (variation can leave attr blank meaning "any")
  ...
  if (!product) return (
    <main className="bg-transparent min-h-screen relative z-10"><Header />
      <div className="py-32 container text-center"><p className="font-display text-2xl text-[#6B1024] font-bold">Product not found.</p>
      <Button asChild className="mt-4 bg-[#6B1024] hover:bg-[#4D0013] text-white"><Link href="/products">Back to Products</Link></Button></div></main>
  )
                ...
                <div key={attr.id} className="pt-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {attr.options.map(opt => {
                      ...
                      // Determine if this option is available given other selections
...
function App() { return <ProductDetail /> }
export default App

...
export async function createProduct(data, user = currentUser) {
...
export async function updateProduct(id, data, user = currentUser) {
  requireRole('editor', user);
...
export async function deleteProduct(id, user = currentUser) {
  requireRole('editor', user);
  const idx = MOCK_PRODUCTS.findIndex((p) => p.id === id);
  if (idx === -1) throw new Error('Product not found');

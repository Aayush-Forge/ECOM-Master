...
export function getRedirectForRole(role, pathname) {
  ...
  // Support clean subdomain paths (e.g. /orders, /overview on admin subdomain)
  const normalizedPath = pathname.startsWith('/admin')
    ? pathname
    : `/admin${pathname.startsWith('/') ? pathname : `/${pathname}`}`;

  // Check if user is in one of their allowed route trees
  const isInAllowedArea =
    allowedPrefixes.some(prefix => pathname.startsWith(prefix)) ||
    (role === 'admin' && (pathname === '/' || allowedPrefixes.some(prefix => normalizedPath.startsWith(prefix))));
